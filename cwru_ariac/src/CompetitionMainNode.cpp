...
int main(int argc, char **argv) {
    ...
    while (ros::ok() && !orderManager.isCompetitionEnd()) {
        ...
        for (auto order : orderManager.orders) {
                	...
                	if(priorityOrderFlag){ break;}//added by Ammar
                  	for (auto object : orderManager.AGVs[useAGV].kitAssigned.objects) {
						...
						while (ros::ok() && !succeed) {
                        	camera.forceUpdate();
                        	PartList allParts = camera.combineLocations(SensorManager::CONVEYOR + SensorManager::BINS+ SensorManager::GROUND, extraParts);
                        	...
                        	if (candidates.size() == 0) {
                            	ROS_WARN("No candidate parts to complete object: %s in kit %s, order %s",object.type.c_str(), kit.kit_type.c_str(), order.order_id.c_str());
							...
							if (robotMove.move(best, target)) {
								ROS_INFO("Successfully moved part to %s, error code is %s", agvName.c_str(),robotMove.getErrorCodeString().c_str());
								...
								if (planningUtils.findDroppedParts(compareSource, compareTarget, wrong, lost,redundant)) {
									...
									for (auto lostPart: lost) {
										...
										orderManager.AGVs[useAGV].kitAssigned.objects.push_back(orderManager.toKitObject(agvName, lostPart));
									}
								...
								orderManager.AGVs[useAGV].kitAssigned.objects.erase(find_if(   orderManager.AGVs[useAGV].kitAssigned.objects.begin(),orderManager.AGVs[useAGV].kitAssigned.objects.end(),[object](osrf_gear::KitObject obj) {
                                  	...
                                  	if (robotMove.pick(badPart)) {
                                      	...
                                      	if (moveResult) {
                                          	if (robotMove.release()) {
                                              	...
                                              	orderManager.AGVs[useAGV].kitCompleted.objects.erase(find_if(orderManager.AGVs[useAGV].kitCompleted.objects.begin(),orderManager.AGVs[useAGV].kitCompleted.objects.end(),[kitObject](osrf_gear::KitObject obj) {
                                              		return obj.type == kitObject.type && matchPose(obj.pose, kitObject.pose);
                                              	}));
                                              	orderManager.AGVs[useAGV].contains.erase(find_if(orderManager.AGVs[useAGV].contains.begin(),orderManager.AGVs[useAGV].contains.end(), [badPart](Part part){
                                              		return isSamePart(badPart, part);
                                              	}));
                                              	continue;
                                          	} else {
                                              	ROS_WARN("Failed to release the gripper, reason: %s",robotMove.getErrorCodeString().c_str());
                                          	}
                                      	} else {
                                          	ROS_WARN("Failed to move robot to AGV1_CRUISE_POSE, reason: %s",robotMove.getErrorCodeString().c_str());
                                      	}
                                  	} else {
                                      	ROS_WARN("Failed to pick the bad part, reason: %s",robotMove.getErrorCodeString().c_str());
                                  	}
                  	...
                  	if(/*TODO: if we detect priority order*/orderManager.priorityOrderReceived==1){
	                  	priorityOrderFlag=true;
	                  	memoryFlag=true;
	                  	ROS_INFO("priority_order received../n holding current order");
	                  	continue;
	                }
                }
            ...
            orderManager.orders.erase(find_if(orderManager.orders.begin(), orderManager.orders.end(),[order](osrf_gear::Order obj) {
