...
LaserScanner::LaserScanner(ros::NodeHandle &nodeHandle):nh(nodeHandle){
	...
	ROS_INFO("subscribers ready.."); 
		...
		cout << "\n\033[0;31mDo you wish to out put images and related .csv files? Press 1 if you do, otherwise press 0: \033[0m";
		cin >> output_flag;
...
void LaserScanner::ForceUpdate() {
	//while (updateCount_cb_speed == checkedCount_cb_speed && updateCount_cb_scan == checkedCount_cb_scan && ros::ok()){
	//	ros::spinOnce();
...
void LaserScanner::integrate_info(const int& part_id, const vector<float>& origin, const vector<float>& pin_origin, const ros::Time& ros_t_stamp, const float& belt_speed){
	...
	cout << "RN item count: " << item_count <<endl;
...
void LaserScanner::type_a_stamped_center(cv::Mat dewarped_mat_c, float& t_0, float& t_1, float& t_stamp, ros::Time& ros_t_0, ros::Time& ros_t_1, ros::Time& ros_t_stamp, vector<float>& origin, vector<int>& origin_pixel, int& part_id) {
	...
	if (output_flag == 1){
		cv::imwrite(imgName_center_a, dewarped_mat_c);
	}
...
void LaserScanner::type_a_asymmetric(cv::Mat dewarped_mat_a, float scan_height, vector<float>& pin_origin, vector<int>& origin_pixel, int part_id) {
	pin_origin.clear();
	...
	// Set a smaller search window based on part type (size) and detected centre location.
	int window_lt = 0;
	int window_rt = 0;
	...
	// searching for the tallest point
		...
		int test_count = 0; //TODO delete after debug
		...
		if (pin_pt_count >= count_target ) {
			...
			x = laser_profiler_2_origin[0] + scan_width_*(pin_j_avg - 50)/101 ; // j: scan img width along world x axis
			y = laser_profiler_2_origin[1] - 0.06 + scan_height*(pin_i_avg - (int)(mat_ht/2) )/mat_ht; // i: scan img height along world x axis
			// z = laser_profiler_2_origin[2] - belt_depth_ + (ht_height * part_z_max)/255; 
			z = laser_profiler_2_origin[2] - belt_depth_ + ht_part;
			...
			if (output_flag == 1){
				cv::imwrite(imgName_pin_a, dewarped_mat_a);
			}
...
void LaserScanner::type_b_stamped_center(cv::Mat dewarped_mat_c, float& t_0, float& t_1, float& t_stamp, ros::Time& ros_t_0, ros::Time& ros_t_1, ros::Time& ros_t_stamp, vector<float>& origin, vector<int>& origin_pixel) {
	...
	// search for a centre of the circular window that contains the square part of the piston rod
...
void LaserScanner::type_b_asymmetric(cv::Mat dewarped_mat_a, float scan_height, vector<float>& pin_origin, vector<int>& origin_pixel, int part_id) {
		...
		if (vec_part_on_belt[0] == false && vec_part_on_belt[1] == true && vec_part_on_belt[2] == true) { // unlock the triple lock
			...
			ros_time_0 = ros_time0_0;
				...
				if (output_flag == 1){
				    	...
				    	// output a height document
				    	for (int i = 0; i <image_x_size; i++){
						for (int j = 0; j < image_y_size; j++){
							writefile << (belt_depth_ - image_z[i][j]) <<",";
						}
						writefile << (belt_depth_ - image_z[i][image_y_size]) << "\n";
				    	}	
					writefile.close();
				}
...
// TODO remove the image and file output/show utilities after debug
				
					if (output_flag == 1){
					    	cv::imshow("original", image);
					    	cv::waitKey(10);
					    	cv::Size s = image.size();
					    	std::cout << "SIZE: "<< s.height << " X " << s.width <<"\n";
					    	cv::imwrite(imgName,  image );

					    	cv::imshow("dewarped", image_resized);
					    	cv::waitKey(10);
					    	cv::Size s_2 = image_resized.size();
					    	std::cout << "DEWARPED_SIZE: "<< s_2.height << " X " << s_2.width <<"\n";
					    	cv::imwrite(imgName_dewarped,  image_resized );
					}
					...
					// output a dewarped depth info csv file
					if (output_flag == 1){
						ofstream writefile_depth(fileName_2);
						for (int i = 0; i <image_resized_x; i++){
							for (int j = 0; j < image_resized_y; j++){
								writefile_depth << (image_resized.at<double>(i, j)) <<",";
						
							}
							writefile_depth << (image_resized.at<double>(i, image_resized_y)) << "\n";
					    	}	
					...
					// Output a id document  
					if (output_flag == 1){
						ofstream writefile_id(fileName_3);
						writefile_id << "Target Number:" << item_count << "\n";
						writefile_id << "Target Id#:" << part_id << "\n";
					
						if (part_id == 1) {writefile_id << "Target Type: Disk Part\n";
						} else if (part_id == 2) {writefile_id << "Target Type: Gasket Part\n";
						} else if (part_id == 3) {writefile_id << "Target Type: Gear Part\n";
						} else if (part_id == 4) {writefile_id << "Target Type: Piston Rod\n";
						} else if (part_id == 999) {writefile_id << "Unknown Object\n";}	
					}


					// Find the a type A part's centre and asymmetric point
					if (part_id == 1 || part_id == 2 || part_id == 3){
						type_a_stamped_center(image_resized, time_0, time_1, t_stamp, ros_time_0, ros_time_1, ros_t_stamp, origin, origin_pixel, part_id);
					...
					ROS_WARN("Huge Noise Detected -- This image is not real.. Discard Image and Continue.");
